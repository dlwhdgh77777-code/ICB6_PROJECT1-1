...
def load_data():
    ...
    # 1. 직장인(배후 수요) 데이터 로드 (깃허브 구조에 맞게 수정)
    worker_path = os.path.join(base_path, 'data/사업체현황(조직형태별_동별)_20260131105815.csv')
    ...
    # 2. 카페(기존 공급) 데이터 로드 (깃허브 구조에 맞게 수정)
    cafe_supply_path = os.path.join(base_path, 'data/서울시_동별_업종별_점포수_상세.csv')
    ...
    # 3. 추정 매출 데이터 로드 (깃허브 구조에 맞게 수정)
    sales_path = os.path.join(base_path, 'data/seoul_cafe_sales_light.csv')
