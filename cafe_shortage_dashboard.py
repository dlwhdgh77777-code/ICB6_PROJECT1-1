...
def load_data():
    ...
    # 3. 추정 매출 데이터 로드 (경량화 파일로 변경)
    sales_path = os.path.join(base_path, '01_data_processing/data/seoul_cafe_sales_light.csv')
    sales_df = pd.read_csv(sales_path, encoding='utf-8-sig') # 생성 시 utf-8-sig 사용
    
    # 이미 '커피-음료'만 필터링된 파일이므로 추가 필터링 불필요
    cafe_sales = sales_df.copy()
...
try:
    ...
    with c3:
        ...
        if not sales_sub.empty:
            avg_sales = sales_sub['점포당평균매출'].mean()
