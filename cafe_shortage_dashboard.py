...
def load_data():
    ...
    # 1. 직장인(배후 수요) 데이터 로드
    worker_path = os.path.join(base_path, '01_data_processing/data/사업체현황(조직형태별_동별)_20260131105815.csv')
    worker_df = pd.read_csv(worker_path, encoding='utf-8', header=None, skiprows=5)
    ...
    # 2. 카페(기존 공급) 데이터 로드 (서울시_동별_업종별_점포수_상세)
    cafe_supply_path = os.path.join(base_path, '01_data_processing/final_data_files/서울시_동별_업종별_점포수_상세.csv')
    cafe_df = pd.read_csv(cafe_supply_path, encoding='utf-8')
    ...
    # 3. 추정 매출 데이터 로드
    sales_path = os.path.join(base_path, '01_data_processing/data/서울시 상권분석서비스(추정매출-행정동)_2024년.csv')
    try:
        # 인코딩 오류 방지를 위해 utf-8 시도 후 cp949 시도
        sales_df = pd.read_csv(sales_path, encoding='utf-8')
    except:
        sales_df = pd.read_csv(sales_path, encoding='cp949')
    
    # '커피-음료' 업종만 필터링 (CS100010)
    cafe_sales = sales_df[sales_df['서비스_업종_코드_명'] == '커피-음료'].copy()
    
    # 행정동별 평균 매출액 계산 (분기별 데이터가 있으므로 동별 평균 활용)
    # 당월_매출_금액: 분기당 매출액이므로 3으로 나누어 월평균 추정
    dong_sales = cafe_sales.groupby('행정동_코드_명').agg({
        '당월_매출_금액': 'mean',
        '당월_매출_건수': 'mean'
    }).reset_index()
    ...
    merged = pd.merge(workers, cafes, on=['자치구', '행정동'], how='inner')
    merged = pd.merge(merged, dong_sales, on='행정동', how='left').fillna(0)
    
    # 5. 특정 지역 제외 (상일2동 제외 요청 반영)
    merged = merged[merged['행정동'] != '상일2동']
    
    # 6. 분석 지표 계산
    # 부족지수 = 종사자수 / (카페수 + 1)
    merged['부족지수'] = merged['종사자수'] / (merged['카페수'] + 1)
    # 점포당 평균 매출액 추정 (행정동 전체 매출 / 점포수)
...
try:
    ...
    with st.expander("💡 데이터 출처 및 지표 안내", expanded=False):
        st.markdown("""
        **1. 사용 데이터**:
        - 배후 수요: `사업체현황(종사자 수)` (2024)
        - 기존 공급: `서울시 동별 점포수 상세(카페)` (2024 하반기)
        - 매출 정보: `서울시 상권분석서비스(추정매출)` (2024)
        
        **2. 주요 지표**:
        - **창업 기회 점수**: 직장인 수 대비 카페 수의 희소성입니다. (높을수록 기회)
        - **점포당 평균 매출**: 해당 동의 카페 전체 매출을 카페 수로 나눈 추정치입니다.
        """)
    ...
    with c3:
        avg_sales = view_df[view_df['점포당평균매출'] > 0]['점포당평균매출'].mean()
        st.metric("평균 점포당 매출", f"{int(avg_sales/10000):,}만원")
    with c4:
        avg_ticket = view_df[view_df['건당평균결제액'] > 0]['건당평균결제액'].mean()
