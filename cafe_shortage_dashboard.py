...
@st.cache_data
def load_data():
    # 1. 직장인(배후 수요) 데이터 로드 (상대 경로로 수정)
    worker_path = '01_data_processing/data/사업체현황(조직형태별_동별)_20260131105815.csv'
    ...
    # 2. 카페(기존 공급) 데이터 로드 (상대 경로로 수정)
    cafe_supply_path = '01_data_processing/final_data_files/서울시_동별_업종별_점포수_상세.csv'
    ...
    # 3. 추정 매출 데이터 로드 (상대 경로로 수정)
    sales_path = '01_data_processing/data/서울시 상권분석서비스(추정매출-행정동)_2024년.csv'
    try:
